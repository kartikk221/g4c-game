...
function spawnNewPeerDrone(coords)
{
    ...
    SPRITES.create("background_drone_" + droneID, dronePlayerGroup, dronePlayerContainer, urltoNativeDirectory("images/drone_" + droneType + "_1.png"), 
    function(sprite){
        ...
        if(droneType == 1){COSTUMES.assign("background_drone_" + droneID, [peerDrone1_1, peerDrone1_2]);}
        if(droneType == 2){COSTUMES.assign("background_drone_" + droneID, [peerDrone2_1, peerDrone2_2]);}
...
function spawnNewBackgroundCloud(coords)
{
    ...
    SPRITES.create("background_cloud_" + cloudID, dronePlayerGroup, dronePlayerContainer, urltoNativeDirectory("images/cloud_" + randomNumber(1, 5) + ".png"), 
    function(sprite){
        sprite.visible = false;
        sprite.anchor.set(0.5);
        sprite.zOrder = randomNumber(1, 9);
...
function enableBackgroundClouds(){
    ...
    {
        spawnNewBackgroundCloud({x: randomNumber(0, fullWidth) - halfwayWidth, y: randomNumber(0, fullHeight * 1.3) - halfwayHeight});
    }
...
function ActivateMovementEngine(){
    ...
    app.ticker.add(function(delta) {
        if(enableUserMovement && !GAME.paused){
                ...
                } else if((dPressed.isDown || RightArrowPressed.isDown) && (playerDrone.x + gameSpeed) < ((1920 / 2) * HSR * 0.95)){
                    xVelocity = gameSpeed;
                    dronePitchForward();
                    DroneFacingRight = true;
                } else if((playerDrone.x - gameSpeed) > ((-1920 / 2) * HSR * 0.95) && (playerDrone.x + gameSpeed) < ((1920 / 2) * HSR * 0.95)){ 
                    smoothDecrease("xVelocity"); dronePitchBackwards(); 
                } else {
                    xVelocity = 0; dronePitchBackwards();
                }
                ...
                [1,2,3,4,5].forEach(function(num){
                    ...
                    if(GAME.missionRequirements["dropoff_" + num] && GAME.missionRequirements["dropoff_" + num] == "incomplete" && distanceBetween < 200){
                        if(distanceBetween < 190){
                            SPRITES.get("delivery_status").visible = true;
                            SPRITES.get("delivery_status").texture = status_delivering;
                            SPRITES.get("delivery_status").moveTo(dropoffWindow.worldPositioning.x, dropoffWindow.worldPositioning.y)
                            if(GAME.deliveryProcess == ""){GAME.deliveryProcess = newDeliveryProcess(GAME.deliveryTime);}
        ...
        if(enablePositioningEngine){
                for (var name in SPRITES.spriteStorage) {
                    if (SPRITES.get(name).positioningEnabled){
                        var sprite = SPRITES.get(name);
                        var newCoordinates = calculateCoordinates({x: sprite.worldPositioning.x, y: sprite.worldPositioning.y});
        ...
        if(backgroundCloudsEngine){
            for (var cloud in backgroundClouds) {
                var cloudObject = SPRITES.get("background_cloud_" + cloud);
                    ...
                    cloudObject.moveTo(cloudObject.worldPositioning.x + backgroundCloudSpeed, cloudObject.worldPositioning.y);
                    } else {
                    cloudObject.moveTo(cloudObject.worldPositioning.x - backgroundCloudSpeed, cloudObject.worldPositioning.y);
                    }
        ...
        if(backgroundDronesEngine && !GAME.paused){
            for (var droneID in backgroundDrones) {
                var droneObject = SPRITES.get("background_drone_" + droneID);
                    droneObject.moveTo(droneObject.worldPositioning.x + droneObject.xMoveSpeed, droneObject.worldPositioning.y + droneObject.yMoveSpeed);
                    if(GAME.missionLastStatus == "active" && Physics.hit(playerDrone, droneObject) && calculateDistance(playerDrone.x, playerDrone.y, droneObject.x, droneObject.y) < 120){
