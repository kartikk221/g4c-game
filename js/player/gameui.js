...
function dialougePrompt(msg, bt, callback) {
    swal({ title: 'Manager', text: msg, icon: '/images/manager.png', allowOutsideClick: false, button: bt }).then(
        (value) => {
            callback();
        }
    );
                                                ...
                                                function () {
                                                    ...
                                                    $('#top_left_ui_window')
                                                        ...
                                                        .each(function () {
                                                            $(this).css(
                                                                'font-size',
                                                                +$(this).css('font-size').replace('px', '') * HSR + 'px'
                                                            );
                                                        }); // Fixes Top-UI being out of scope.
                                                }
                                            );
                                        }
                                    );
    ...
    function getNextDeliveryFloor() {
        for (var dropoffwindow in GAME.missionRequirements) {
            if (GAME.missionRequirements[dropoffwindow] == 'incomplete') {
                return (
                    Math.floor(
                        (SPRITES.get(dropoffwindow.replace('dropoff_', 'dropoff_window_')).worldPositioning.y - 255) /
                            420
                    ) + 1
                );
            }
    ...
    mission_setup: {
        job1: function () {
            GAME.missionObjective =
                'You are tasked to deliver a package to one of our customers on floor #2. To deliver a package, simply go near the dropoff window and wait for the delivering process to finish.';
            bringDroneToPickupPoint();
            ...
            GAME.onPackageDelivery = function (droffoffwindowNum) {
                ...
                setTimeout(function () {
                    throwMissionSuccessPrompt(
                        'Manager: Congrats on finishing job #1. I would have never thought that the skyscraper industry would allow us to operate drones remotely and deliver products. Anyways, The next job will have increased difficulty as other drones will start moving at faster speeds',
                        'Job #2 has been unlocked'
                    );
                });
            };
            screenFadeIn();
        ...
        job2: function () {
            GAME.missionObjective =
                'Your new task is to deliver two packages to our customers on floors #2 and #6. To deliver a package, simply go near the dropoff window and wait for the delivering process to finish.';
            bringDroneToPickupPoint();
            ...
            GAME.onPackageDelivery = function (droffoffwindowNum) {
                if (
                    GAME.missionRequirements['dropoff_1'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_2'] == 'delivered'
                ) {
                    GAME.missionLastStatus = 'complete';
                    ...
                    setTimeout(function () {
                        throwMissionSuccessPrompt(
                            'Manager: Wow! You are a fast learner! Congrats on finishing job #2. Remember this is the future, drone traffic has increased in our airpsace but it also causes a higher risk of drone-human accidents. Be careful! The next job will have increased difficulty as other drones will start moving at faster speeds.',
                            'Job #3 has been unlocked'
                        );
                    });
                } else {
        ...
        job3: function () {
            GAME.missionObjective =
                'This task requires you to deliver three packages to our customers on floors #2, #6, and #12. To deliver a package, simply go near the dropoff window and wait for the delivering process to finish.';
            bringDroneToPickupPoint();
            GAME.deliveryTime = 120;
            GAME.missionRequirements['dropoff_1'] = 'incomplete';
            GAME.missionRequirements['dropoff_2'] = 'incomplete';
            ...
            GAME.onPackageDelivery = function (droffoffwindowNum) {
                if (
                    GAME.missionRequirements['dropoff_1'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_2'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_3'] == 'delivered'
                ) {
                    GAME.missionLastStatus = 'complete';
                    ...
                    setTimeout(function () {
                        throwMissionSuccessPrompt(
                            'Manager: Good Job! It seems you have learned to manage your time efficiently while operating through fast drone traffic! Congrats on finishing job #3. The next job will have increased difficulty as other drones will start moving at faster speeds.',
                            'Job #4 has been unlocked'
                        );
                    });
                } else {
                    updatePackageDimensions(droneDimensionsCategory - 1);
        ...
        job4: function () {
            GAME.missionObjective =
                'You must deliver four packages to our customers on floors #2, #6, #12, and #15. To deliver a package, simply go near the dropoff window and wait for the delivering process to finish.';
            bringDroneToPickupPoint();
            GAME.deliveryTime = 120;
            GAME.missionRequirements['dropoff_1'] = 'incomplete';
            GAME.missionRequirements['dropoff_2'] = 'incomplete';
            ...
            GAME.onPackageDelivery = function (droffoffwindowNum) {
                if (
                    ...
                    GAME.missionRequirements['dropoff_2'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_3'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_4'] == 'delivered'
                ) {
                    GAME.missionLastStatus = 'complete';
        ...
        job5: function () {
            GAME.missionObjective =
                'You must deliver five packages to our customers on floors #2, #6, #12, #15, and #17. To deliver a package, simply go near the dropoff window and wait for the delivering process to finish.';
            bringDroneToPickupPoint();
            ...
            GAME.onPackageDelivery = function (droffoffwindowNum) {
                if (
                    ...
                    GAME.missionRequirements['dropoff_3'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_4'] == 'delivered' &&
                    GAME.missionRequirements['dropoff_5'] == 'delivered'
                ) {
                    GAME.missionLastStatus = 'complete';
    ...
    updateMissionsUI: function () {
        for (var job in GAME.jobs_status) {
            ...
            if (status == 'unlocked') {
                ...
                jobEl
                    .find('.mission-disclaimer')
                    .find('strong')
                    .text('Please complete previous jobs to unlock this job!');
